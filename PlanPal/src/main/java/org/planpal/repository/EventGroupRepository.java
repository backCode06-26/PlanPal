...
public class EventGroupRepository {
    ...
    private List<Integer> getEventGroupsByRatingIdDesc(int page) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try {
            ...
            String sql = "SELECT * FROM ( " +
                    "    SELECT eg.group_id, AVG(r.rating) AS avg_rating, ROW_NUMBER() OVER (ORDER BY AVG(r.rating) DESC) AS num " +
                    "    FROM event_groups eg " +
                    "    JOIN reviews r ON eg.group_id = r.group_id " +
            ...
            throw new RuntimeException("Error retrieving event groups sorted by rating with pagination", e);
        } finally {
            DataBaseUtils.close(conn, ps, rs);
        }
    ...
    public List<EventGroupDTO> getEventGroupsByRatingDesc(int page) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Integer> groupIds = getEventGroupsByRatingIdDesc(page);
        List<EventGroupDTO> eventGroups = new ArrayList<>();
        ...
        try {
            ...
            StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM event_groups WHERE group_id IN (");
